// c++ -o lin lin.cpp `root-config --glibs --cflags`
...
int main (int argc, char ** argv)
{
	if (argc < 2)
  	...
  	myGraph->SetTitle("LYSO Energy Calibration");
  	myGraph->GetXaxis()->SetTitle("Energy [keV]");
  	myGraph->GetYaxis()->SetTitle("ADC Channels");
  	
  	TLatex *latex = new TLatex(myGraph->GetX()[0], myGraph->GetY()[0],"^{57}Co"); 
  	TLatex *latex1 = new TLatex(myGraph->GetX()[1], myGraph->GetY()[1],"^{57}Co"); 
  	TLatex *latex2 = new TLatex(myGraph->GetX()[2], myGraph->GetY()[2],"^{22}Na"); 
  	TLatex *latex3 = new TLatex(myGraph->GetX()[3], myGraph->GetY()[3],"^{22}Na"); 
